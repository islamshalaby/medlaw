                    ...
                    <div class="row">
                        ...
                        <form style="width: 100%" id="areaForm" method="">
                            <div class="row">
                                ...
                                <div class="form-group col-md-3">
                                    <label for="dr_lawyer">{{ __('messages.doctor|lawyer') }}</label>
                                    <select id="area_select" name="dr_lawyer" class="form-control">
                                        <option disabled selected>{{ __('messages.select') }}</option>
                                        @foreach ( $data['doctors_lawyers'] as $drs_lawyers )
                                        <option {{ isset($data['dr_law']) && $data['dr_law'] == $drs_lawyers->id ? 'selected' : '' }} value="{{ $drs_lawyers->id }}">{{ App::isLocale('en') ? $drs_lawyers->app_name_en : $drs_lawyers->app_name_ar }}</option>
                                        @endforeach 
                                    </select>
                                </div>
                                <div class="form-group col-md-3">
                                    <label for="dr_lawyer">{{ __('messages.type') }}</label>
                                    <select name="type" class="form-control" >
                                        <option selected disabled >{{ __('messages.select') }}</option>
                                        <option {{ isset($data['type']) && $data['type'] == "doctor" ? 'selected' : '' }} value="doctor" >{{ __('messages.doctor') }}</option>
                                        <option {{ isset($data['type']) && $data['type'] == "lawyer" ? 'selected' : '' }} value="lawyer" >{{ __('messages.lawyer') }}</option>
                                    </select>
                                </div>
                                <div class="form-group col-md-3">
                ...
                <table id="order-tbl" class="table table-hover non-hover" style="width:100%">
                    <thead>
                        <tr>
                            ...
                            <th>{{ __('messages.doctor|lawyer') }}</th>
                    ...
                    <tbody>
                        ...
                        @foreach ($data['reservations'] as $reservation)
                            <tr>
                                ...
                                <td>{{ App::isLocale('en') ? $reservation->doctorLawyer->app_name_en : $reservation->doctorLawyer->app_name_ar }}</td>
                                <td>{{ $reservation->type == 'doctor' ? __('messages.doctor') : __('messages.lawyer') }}</td>
                                <td>
                                ...
                                @if(Auth::user()->update_data) 
                                    <td class="text-center blue-color" ><a href="{{ route('services.details', $reservation->id) }}" ><i class="far fa-eye"></i></a></td>
                                @endif
